...
const UpdateModal = ({ updateclose, update, id, tableRenderTrue }) => {
  ...
  const [course_track, setUser] = useState({})
  const showDetail = async (details) => {
    console.log('hai' + details)
    try {
      const response = await axios.get(`http://localhost:8000/course_track/${details}`)
  ...
  const updateUser = async (course_track) => {
    const { _id, name, desc,status } = course_track
    try {
      const course_track = {
        name: name,
        desc:"desc",
      ...
      const response = await axios.patch(`http://localhost:8000/course_track/${_id}`, course_track)
              ...
              <Col lg={12}>
                <Form action="">
                  ...
                  >
                    ...
                    <option
                      className={course_track.status === 'Active' ? 'd-none' : 'd-block'}
                      value="Active"
                    >
                      Active
                    </option>
                    <option
                      className={course_track.status === 'Inactive' ? 'd-none' : 'd-block'}
                      value="Inactive"
                    >
                      Inactive
                    </option>
