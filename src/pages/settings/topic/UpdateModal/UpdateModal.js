...
const UpdateModal = ({ updateclose, update, id, tableRenderTrue }) => {
  ...
  const updateUser = async (course_topic) => {
    const { _id, name, desc,status } = course_topic
    try {
      const course_topic = {
        name: name,
        desc:"desc",
      ...
      const response = await axios.patch(`http://localhost:8000/course_topic/${_id}`, course_topic)
              ...
              <Col lg={12}>
                <Form action="">
                  ...
                  >
                    ...
                    <option
                      className={course_topic.status === 'Active' ? 'd-none' : 'd-block'}
                      value="Active"
                    >
                      Active
                    </option>
                    <option
                      className={course_topic.status === 'Inactive' ? 'd-none' : 'd-block'}
                      value="Inactive"
                    >
                      Inactive
                    </option>
