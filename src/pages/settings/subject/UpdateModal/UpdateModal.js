...
const UpdateModal = ({ updateclose, update, id, tableRenderTrue }) => {
  ...
  const [course_subject, setUser] = useState({})
  const showDetail = async (details) => {
    console.log('hai' + details)
    try {
      const response = await axios.get(`http://localhost:8000/course_subject/${details}`)
  ...
  const updateUser = async (course_subject) => {
    const { _id, name, desc,status } = course_subject
    try {
      ...
      const response = await axios.patch(`http://localhost:8000/course_subject/${_id}`, course_subject)
              ...
              <Col lg={12}>
                <Form action="">
                  ...
                  >
                    ...
                    <option
                      className={course_subject.status === 'Active' ? 'd-none' : 'd-block'}
                      value="Active"
                    >
                      Active
                    </option>
                    <option
                      className={course_subject.status === 'Inactive' ? 'd-none' : 'd-block'}
                      value="Inactive"
                    >
                      Inactive
                    </option>
